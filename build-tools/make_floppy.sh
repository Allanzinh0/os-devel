...
dd if=${BUILD_DIR}/bootloader/stage1.bin of=$TARGET conv=notrunc bs=1 count=3 >/dev/null
dd if=${BUILD_DIR}/bootloader/stage1.bin of=$TARGET conv=notrunc bs=1 seek=$STAGE1_OFFSET skip=$STAGE1_OFFSET >/dev/null
...
mmd -i $TARGET "::boot"
mcopy -i $TARGET ${BUILD_DIR}/kernel/kernel.elf "::boot/kernel.elf"
