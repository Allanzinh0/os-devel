...
dd if=${OBJ_DIR}/stage1.bin of=$TARGET_PARTITION conv=notrunc bs=1 count=3 >/dev/null
dd if=${OBJ_DIR}/stage1.bin of=$TARGET_PARTITION conv=notrunc bs=1 seek=$STAGE1_OFFSET skip=$STAGE1_OFFSET >/dev/null
