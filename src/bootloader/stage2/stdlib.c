...
void qsort_internal(void *base, size_t num, size_t size, size_t left,
                    size_t right, int (*compar)(const void *, const void *)) {
  if (left >= right)
    return;
  ...
  for (;;) {
    ...
    for (int k = 0; k < size; k++) {
      ...
      *((uint8_t *)(base + (i * size)) + k) =
          *((uint8_t *)(base + (j * size)) + k);
